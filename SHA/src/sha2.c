#include <sha2.h>

#include <stdlib.h>
#include <string.h>

#include <sysmacros.h>

static const uint32_t constants_table_32[64] = {
    0x428a2f98, 0x71374491, 0xb5c0fbcf, 0xe9b5dba5,
    0x3956c25b, 0x59f111f1, 0x923f82a4, 0xab1c5ed5,
    0xd807aa98, 0x12835b01, 0x243185be, 0x550c7dc3,
    0x72be5d74, 0x80deb1fe, 0x9bdc06a7, 0xc19bf174,
    0xe49b69c1, 0xefbe4786, 0x0fc19dc6, 0x240ca1cc,
    0x2de92c6f, 0x4a7484aa, 0x5cb0a9dc, 0x76f988da,
    0x983e5152, 0xa831c66d, 0xb00327c8, 0xbf597fc7,
    0xc6e00bf3, 0xd5a79147, 0x06ca6351, 0x14292967,
    0x27b70a85, 0x2e1b2138, 0x4d2c6dfc, 0x53380d13,
    0x650a7354, 0x766a0abb, 0x81c2c92e, 0x92722c85,
    0xa2bfe8a1, 0xa81a664b, 0xc24b8b70, 0xc76c51a3,
    0xd192e819, 0xd6990624, 0xf40e3585, 0x106aa070,
    0x19a4c116, 0x1e376c08, 0x2748774c, 0x34b0bcb5,
    0x391c0cb3, 0x4ed8aa4a, 0x5b9cca4f, 0x682e6ff3,
    0x748f82ee, 0x78a5636f, 0x84c87814, 0x8cc70208,
    0x90befffa, 0xa4506ceb, 0xbef9a3f7, 0xc67178f2
};

static const uint64_t constants_table_64[80] = {
    0x428a2f98d728ae22, 0x7137449123ef65cd, 0xb5c0fbcfec4d3b2f, 0xe9b5dba58189dbbc,
    0x3956c25bf348b538, 0x59f111f1b605d019, 0x923f82a4af194f9b, 0xab1c5ed5da6d8118,
    0xd807aa98a3030242, 0x12835b0145706fbe, 0x243185be4ee4b28c, 0x550c7dc3d5ffb4e2,
    0x72be5d74f27b896f, 0x80deb1fe3b1696b1, 0x9bdc06a725c71235, 0xc19bf174cf692694,
    0xe49b69c19ef14ad2, 0xefbe4786384f25e3, 0x0fc19dc68b8cd5b5, 0x240ca1cc77ac9c65,
    0x2de92c6f592b0275, 0x4a7484aa6ea6e483, 0x5cb0a9dcbd41fbd4, 0x76f988da831153b5,
    0x983e5152ee66dfab, 0xa831c66d2db43210, 0xb00327c898fb213f, 0xbf597fc7beef0ee4,
    0xc6e00bf33da88fc2, 0xd5a79147930aa725, 0x06ca6351e003826f, 0x142929670a0e6e70,
    0x27b70a8546d22ffc, 0x2e1b21385c26c926, 0x4d2c6dfc5ac42aed, 0x53380d139d95b3df,
    0x650a73548baf63de, 0x766a0abb3c77b2a8, 0x81c2c92e47edaee6, 0x92722c851482353b,
    0xa2bfe8a14cf10364, 0xa81a664bbc423001, 0xc24b8b70d0f89791, 0xc76c51a30654be30,
    0xd192e819d6ef5218, 0xd69906245565a910, 0xf40e35855771202a, 0x106aa07032bbd1b8,
    0x19a4c116b8d2d0c8, 0x1e376c085141ab53, 0x2748774cdf8eeb99, 0x34b0bcb5e19b48a8,
    0x391c0cb3c5c95a63, 0x4ed8aa4ae3418acb, 0x5b9cca4f7763e373, 0x682e6ff3d6b2b8a3,
    0x748f82ee5defb2fc, 0x78a5636f43172f60, 0x84c87814a1f0ab72, 0x8cc702081a6439ec,
    0x90befffa23631e28, 0xa4506cebde82bde9, 0xbef9a3f7b2c67915, 0xc67178f2e372532b,
    0xca273eceea26619c, 0xd186b8c721c0c207, 0xeada7dd6cde0eb1e, 0xf57d4f7fee6ed178,
    0x06f067aa72176fba, 0x0a637dc5a2c898a6, 0x113f9804bef90dae, 0x1b710b35131c471b,
    0x28db77f523047d84, 0x32caab7b40c72493, 0x3c9ebe0a15c9bebc, 0x431d67c49c100d4c,
    0x4cc5d4becb3e42b6, 0x597f299cfc657e2a, 0x5fcb6fab3ad6faec, 0x6c44198c4a475817
};

void SHA256_init(SHA256_context *context) {
    context->size = 0;
    context->buffer_size = 0;

    context->state[0] = 0x6a09e667;
    context->state[1] = 0xbb67ae85;
    context->state[2] = 0x3c6ef372;
    context->state[3] = 0xa54ff53a;
    context->state[4] = 0x510e527f;
    context->state[5] = 0x9b05688c;
    context->state[6] = 0x1f83d9ab;
    context->state[7] = 0x5be0cd19;
}

void SHA224_init(SHA224_context *context) {
    context->size = 0;
    context->buffer_size = 0;

    context->state[0] = 0xc1059ed8;
    context->state[1] = 0x367cd507;
    context->state[2] = 0x3070dd17;
    context->state[3] = 0xf70e5939;
    context->state[4] = 0xffc00b31;
    context->state[5] = 0x68581511;
    context->state[6] = 0x64f98fa7;
    context->state[7] = 0xbefa4fa4;
}

void SHA512_init(SHA512_context *context) {
    context->size[0] = 0;
    context->size[1] = 0;
    context->buffer_size = 0;

    context->state[0] = 0x6a09e667f3bcc908;
    context->state[1] = 0xbb67ae8584caa73b;
    context->state[2] = 0x3c6ef372fe94f82b;
    context->state[3] = 0xa54ff53a5f1d36f1;
    context->state[4] = 0x510e527fade682d1;
    context->state[5] = 0x9b05688c2b3e6c1f;
    context->state[6] = 0x1f83d9abfb41bd6b;
    context->state[7] = 0x5be0cd19137e2179;
}

void SHA384_init(SHA384_context *context) {
    context->size[0] = 0;
    context->size[1] = 0;
    context->buffer_size = 0;

    context->state[0] = 0xcbbb9d5dc1059ed8;
    context->state[1] = 0x629a292a367cd507;
    context->state[2] = 0x9159015a3070dd17;
    context->state[3] = 0x152fecd8f70e5939;
    context->state[4] = 0x67332667ffc00b31;
    context->state[5] = 0x8eb44a8768581511;
    context->state[6] = 0xdb0c2e0d64f98fa7;
    context->state[7] = 0x47b5481dbefa4fa4;
}

#define ROTR32(x, n) (((x) >> (n)) | ((x) << (32 - (n))))

#define SSIG0(x) (ROTR32(x, 7) ^ ROTR32(x, 18) ^ ((x) >> 3))
#define SSIG1(x) (ROTR32(x, 17) ^ ROTR32(x, 19) ^ ((x) >> 10))
#define BSIG0(x) (ROTR32(x, 2) ^ ROTR32(x, 13) ^ ROTR32(x, 22))
#define BSIG1(x) (ROTR32(x, 6) ^ ROTR32(x, 11) ^ ROTR32(x, 25))

#define CH(x, y, z) (((x) & (y)) ^ (~(x) & (z)))
#define MAJ(x, y, z) (((x) & (y)) ^ ((x) & (z)) ^ ((y) & (z)))

static void processMessage32(SHA256_context *context) {
    uint32_t schedule[64];
    for (uint8_t i = 0; i < 16; i++) {
        schedule[i] = context->buffer[i];
    }
    for (uint8_t i = 16; i < 64; i++) {
        const uint32_t a = schedule[i - 2];
        const uint32_t b = schedule[i - 15];
        schedule[i] = SSIG1(a) + SSIG0(b) + schedule[i - 7] + schedule[i - 16];
    }

    uint32_t a = context->state[0], b = context->state[1];
    uint32_t c = context->state[2], d = context->state[3];
    uint32_t e = context->state[4], f = context->state[5];
    uint32_t g = context->state[6], h = context->state[7];

    for (uint8_t i = 0; i < 64; i++) {
        uint32_t t1 = h + BSIG1(e) + CH(e, f, g) + constants_table_32[i] + schedule[i];
        uint32_t t2 = BSIG0(a) + MAJ(a, b, c);
        h = g;
        g = f;
        f = e;
        e = d + t1;
        d = c;
        c = b;
        b = a;
        a = t1 + t2;
    }

    context->state[0] += a; context->state[1] += b;
    context->state[2] += c; context->state[3] += d;
    context->state[4] += e; context->state[5] += f;
    context->state[6] += g; context->state[7] += h;
}

int SHA256_update(SHA256_context *context, const uint8_t *data, uint64_t bits) {
    uint8_t buffer_loc = context->buffer_size / 32;
    uint8_t buffer_size = context->buffer_size % 32;
    uint32_t buffer = buffer_size ? context->buffer[buffer_loc] & N_BIT_MASK32_LFT(buffer_size) : 0;

    if (bits > UINT64_MAX - context->size) return -1;
    context->size += bits;

    while (bits) {
        const uint8_t byte = bits < 8 ? *data & N_BIT_MASK8_LFT(bits) : *data;
        const uint8_t byte_size = bits < 8 ? bits : 8;
        buffer |= buffer_size > 24 ? (uint32_t) byte >> (buffer_size - 24) : (uint32_t) byte << (24 - buffer_size);
        buffer_size += byte_size;
        if (buffer_size >= 32) {
            context->buffer[buffer_loc++] = buffer;
            if (buffer_loc == 16) {
                buffer_loc = 0;
                processMessage32(context);
            }
            buffer_size -= 32;
            buffer = buffer_size ? (uint32_t) byte << (24 + byte_size - buffer_size) : 0;
        }
        bits -= byte_size;
        data++;
    }

    context->buffer_size = buffer_loc * 32 + buffer_size;
    if (buffer_size) {
        context->buffer[buffer_loc] = buffer;
    }

    return 0;
}

static void finaliseMessage32(SHA256_context *context) {
    const uint64_t final_size = context->size;
    context->size = 0;
    
    const uint8_t data = 128;
    SHA256_update(context, &data, 1);
    uint8_t buffer_words = INT_DIV_CEIL(context->buffer_size, 32);

    if (buffer_words > 14) {
        memset(context->buffer + buffer_words, 0, 64 - buffer_words * 4);
        processMessage32(context);
        buffer_words = 0;
    }

    memset(context->buffer + buffer_words, 0, 56 - buffer_words * 4);
    context->buffer[14] = (uint32_t) (final_size >> 32);
    context->buffer[15] = (uint32_t) final_size;
    processMessage32(context);
}

void SHA256_close(SHA256_context *context, uint8_t hash[32]) {
    finaliseMessage32(context);
    for (uint8_t i = 0; i < 32; i++) {
        hash[i] = NTH_MS_BYTE32(context->state[i / 4], i % 4);
    }
}

void SHA224_close(SHA224_context *context, uint8_t hash[28]) {
    finaliseMessage32(context);
    for (uint8_t i = 0; i < 28; i++) {
        hash[i] = NTH_MS_BYTE32(context->state[i / 4], i % 4);
    }
}